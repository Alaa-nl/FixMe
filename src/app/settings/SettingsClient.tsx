...
export default function SettingsClient({ content }: SettingsClientProps) {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      setPasswordMessage(content["settings_error_mismatch"]);
      return;
    }

    if (newPassword.length < 8) {
      setPasswordMessage(content["settings_error_password_short"]);
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setPasswordMessage(content["settings_password_success"]);
        setCurrentPassword("");
        setNewPassword("");
        setConfirmPassword("");
        setTimeout(() => {
        ...
        setPasswordMessage(data.error || content["settings_error_password"]);
      }
    } catch (error) {
      console.error("Error changing password:", error);
      setPasswordMessage(content["settings_error_generic"]);
    } finally {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        alert(data.error || content["settings_error_generic"]);
      }
    } catch (error) {
      console.error("Error deleting account:", error);
      alert(content["settings_error_generic"]);
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  if (!user) {
    return (
      <div className="min-h-screen pt-20 bg-gray-50 flex items-center justify-center">
        <div className="text-gray-600">{content["settings_loading"]}</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pt-20 bg-gray-50">
        ...
        <div className="bg-white rounded-xl shadow-sm border divide-y">
          ...
          <div className="p-6 space-y-4">
            <h2 className="text-xl font-bold text-gray-800">
              {content["settings_account_title"]}
            </h2>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {content["settings_email_label"]}
              </label>
              <input
                type="email"
                value={user.email}
            ...
            {canChangePassword && (
              <div>
                ...
                >
                  {content["settings_change_password"]}
                  {showPasswordChange ? (
                  ...
                  <form onSubmit={handlePasswordChange} className="mt-4 space-y-3">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {content["settings_current_password"]}
                      </label>
                      <PasswordInput
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {content["settings_confirm_password"]}
                      </label>
                      <PasswordInput
                    ...
                    >
                      {isChangingPassword
                        ? content["settings_password_updating"]
                        : content["settings_password_update"]}
                    </button>
                  </form>
                )}
              </div>
            ...
            <div>
              <p className="text-sm font-medium text-gray-700 mb-3">
                {content["settings_connected_accounts"]}
              </p>
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  {isGoogleAccount ? (
                    <span className="px-3 py-1 bg-green-100 text-green-800 text-xs font-semibold rounded-full">
                      {content["settings_connected_badge"]}
                    </span>
                  ) : (
                    <button className="px-3 py-1 border border-gray-300 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-50 transition-colors">
                      {content["settings_connect_button"]}
                    </button>
                  )}
                </div>
          ...
          <div className="p-6 space-y-4">
            <h2 className="text-xl font-bold text-gray-800">
              {content["settings_notif_title"]}
            </h2>

            <div className="space-y-4">
              <Toggle
                label={content["settings_notif_offers"]}
                description={content["settings_notif_offers_desc"]}
                checked={emailOffers}
                onChange={(checked) => {
              ...
              <Toggle
                label={content["settings_notif_messages"]}
                description={content["settings_notif_messages_desc"]}
              ...
              <Toggle
                label={content["settings_notif_jobs"]}
                description={content["settings_notif_jobs_desc"]}
              ...
              <Toggle
                label={content["settings_notif_reviews"]}
                description={content["settings_notif_reviews_desc"]}
            ...
            <p className="text-xs text-gray-500 mt-4">
              {content["settings_notif_inapp_note"]}
            </p>
          </div>
          ...
          <div className="p-6 space-y-4">
            <h2 className="text-xl font-bold text-gray-800">
              {content["settings_privacy_title"]}
            </h2>

            <div className="space-y-4">
              <Toggle
                label={content["settings_privacy_city"]}
                description={content["settings_privacy_city_desc"]}
                checked={showCity}
                onChange={(checked) => {
                  setShowCity(checked);
              ...
              <Toggle
                label={content["settings_privacy_phone"]}
                description={content["settings_privacy_phone_desc"]}
                checked={showPhone}
                onChange={(checked) => {
          ...
          <div className="p-6 space-y-4">
            <h2 className="text-xl font-bold text-gray-800">
              {content["settings_language_title"]}
            </h2>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {content["settings_language_label"]}
              </label>
              <select
                value={language}
                onChange={(e) => {
                ...
                className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              >
                <option value="English">{content["settings_language_english"]}</option>
                <option value="Nederlands">{content["settings_language_dutch"]}</option>
              </select>
            </div>
          </div>
          ...
          <div className="p-6 space-y-4 border-2 border-red-200 rounded-b-xl">
            ...
            <div>
              ...
              >
                {content["settings_delete_button"]}
              </button>
            </div>
          </div>
        </div>
      ...
      <Modal
        ...
        title={content["settings_delete_modal_title"]}
      >
        <div className="space-y-4">
          <p className="text-gray-700">
            {content["settings_delete_modal_body"]}
          </p>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {content["settings_delete_confirm_label"]}
            </label>
            <input
              type="text"
          ...
          >
            {isDeleting ? content["settings_deleting"] : content["settings_delete_button"]}
          </button>
        </div>
      </Modal>
    </div>
